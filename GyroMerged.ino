...
#define Addr_Accl 0x1E
    int MPU_ACC_SCALE_FACTOR = 0.488281;
    int MPU_GYRO_SCALE_FACTOR = 0.060975;
int xAccl = 0, yAccl =  0, zAccl = 0, xGyro = 0, yGyro = 0, zGyro = 0, xMag = 0, yMag = 0, zMag = 0;
int  tempXA = 0,  tempXY = 0,  tempXZ = 0,  tempXG = 0,  tempXY = 0,  tempXZ = 0
void setup() 
{
    // Set variable
...
void loop()
{
    ...
    { 
    tempXG = (data[0] << 8) | data[1];
    *xGyro = tempXG * MPU_GYRO_SCALE_FACTOR;
    tempYG = (data[2] << 8) | data[3];
    *yGyro = tempYG * MPU_GYRO_SCALE_FACTOR;
    tempZG = (data[4] << 8) | data[5]);
    *zGyro = tempZG * MPU_GYRO_SCALE_FACTOR;
    }
    for(int i = 0; i < 6; i++)
    {
        // Start I2C Transmission
    ...
    tempXA = ((data[0] << 8) | data[1]));
    *xAccl = temp * MPU_ACC_SCALE_FACTOR;
    tempYA = ((data[2] << 8) | data[3]));
    *yAccl = temp * MPU_ACC_SCALE_FACTOR;
    tempZA = ((data[4] << 8) | data[5]));
    *zAccl = temp * MPU_ACC_SCALE_FACTOR;
